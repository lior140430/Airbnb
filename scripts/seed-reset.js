 ...
 *  - Creates 10 properties: 5 per user, 4 with images, 6 without
 *
...
// 4 properties with images (3 photos each)
const IMAGE_SETS = [
    [
...
const PROPERTIES = [
    ...
    {
        title: 'סטודיו מודרני במרכז העיר',
...
async function main() {
    ...
    for (let i = 0; i < PROPERTIES.length; i++) {
        ...
        const hasImg = p.images.length > 0 ? `${p.images.length} photos` : 'NO IMAGES';
        console.log(`  [${i + 1}] ${p.title} → ${i < 5 ? 'ליאור' : 'רוני'} (${hasImg})`);
    }
    ...
    console.log('──────────────────────────────');
    console.log('Properties with images : 4 (props 1,2,6,7)');
    console.log('Properties without img : 6 (props 3,4,5,8,9,10)');
